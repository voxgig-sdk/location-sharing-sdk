...
The Lua SDK for the LocationSharing API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Address(nil):load({ id = "example_id" }, nil)
...
local result, err = client:LocationSharing(nil):load(
  { id = "test01" }, nil
)
