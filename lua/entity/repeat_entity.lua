...
local RepeatEntity = {}
RepeatEntity.__index = RepeatEntity


function RepeatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepeatEntity)
  self._name = "repeat"
  self._client = client
...
function RepeatEntity:get_name()
  return self._name
end


function RepeatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepeatEntity.new(self._client, opts)
end
...
function RepeatEntity:mark_deleted()
  self._deleted = true
end


function RepeatEntity:deleted()
  return true == self._deleted
end


function RepeatEntity:data_set(args)
  if args ~= nil then
...
function RepeatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepeatEntity:match_set(args)
  if args ~= nil then
...
function RepeatEntity:match_get()
...
function RepeatEntity:stream(action, args, callopts)
...
function RepeatEntity:_run_op(ctx, post_done)
  local utility = self._utility
