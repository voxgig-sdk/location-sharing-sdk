...
local ShareEntity = {}
ShareEntity.__index = ShareEntity


function ShareEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShareEntity)
  self._name = "share"
  self._client = client
...
function ShareEntity:get_name()
  return self._name
end


function ShareEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShareEntity.new(self._client, opts)
end
...
function ShareEntity:mark_deleted()
  self._deleted = true
end


function ShareEntity:deleted()
  return true == self._deleted
end


function ShareEntity:data_set(args)
  if args ~= nil then
...
function ShareEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShareEntity:match_set(args)
  if args ~= nil then
...
function ShareEntity:match_get()
...
function ShareEntity:stream(action, args, callopts)
...
function ShareEntity:_run_op(ctx, post_done)
  local utility = self._utility
