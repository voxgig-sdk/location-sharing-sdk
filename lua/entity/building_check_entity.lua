-- LocationSharing SDK BuildingCheck entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local BuildingCheckEntity = {}
BuildingCheckEntity.__index = BuildingCheckEntity


function BuildingCheckEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BuildingCheckEntity)
  self._name = "building_check"
  self._client = client
...
function BuildingCheckEntity:get_name()
  return self._name
end


function BuildingCheckEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BuildingCheckEntity.new(self._client, opts)
end
...
function BuildingCheckEntity:mark_deleted()
  self._deleted = true
end


function BuildingCheckEntity:deleted()
  return true == self._deleted
end


function BuildingCheckEntity:data_set(args)
  if args ~= nil then
...
function BuildingCheckEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BuildingCheckEntity:match_set(args)
  if args ~= nil then
...
function BuildingCheckEntity:match_get()
...
function BuildingCheckEntity:stream(action, args, callopts)
...
---@param reqmatch BuildingCheckListMatch
---@param ctrl? table
---@return BuildingCheck[]
---@return string? err
function BuildingCheckEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function BuildingCheckEntity:_run_op(ctx, post_done)
  local utility = self._utility
