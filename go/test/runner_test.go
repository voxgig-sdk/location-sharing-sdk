...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("LOCATION_SHARING_TEST_LIVE") == "TRUE" ||
		os.Getenv("LOCATION_SHARING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("LOCATION_SHARING_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.LocationSharingSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.LocationSharingError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.LocationSharingSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.LocationSharingError{Msg: msg, Code: code}
}
