...
func TestLocationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LOCATION_SHARING_TEST_LOCATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if locationRef01DataDt0Loaded == nil {
...
func locationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LOCATION_SHARING_TEST_LOCATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LOCATION_SHARING_TEST_LOCATION_ENTID"])
