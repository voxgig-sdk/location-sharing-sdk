...
func TestBuildingCheckEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.BuildingCheck(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.BuildingCheck(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		buildingCheckRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.building_check")))
...
func building_checkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LOCATION_SHARING_TEST_BUILDING_CHECK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LOCATION_SHARING_TEST_BUILDING_CHECK_ENTID"])
