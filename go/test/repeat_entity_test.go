...
func TestRepeatEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		repeatRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "repeat"}), "repeat_ref01"))

		repeatRef01DataResult, err := repeatRef01Ent.Create(repeatRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		repeatRef01Data = core.ToMapAny(entityData(repeatRef01DataResult))
...
func repeatBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LOCATION_SHARING_TEST_REPEAT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LOCATION_SHARING_TEST_REPEAT_ENTID"])
