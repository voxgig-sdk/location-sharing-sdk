...
import (
	...
	omni "github.com/voxgig-sdk/location-sharing-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LocationSharingError); is {
				out := map[string]any{
					"name":    "LocationSharingError",
...
func omniCtx(arg any, client *sdk.LocationSharingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LocationSharingSDK); is {
			client = live
