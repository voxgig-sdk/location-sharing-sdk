...
func TestShareEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		shareRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "share"}), "share_ref01"))

		shareRef01DataResult, err := shareRef01Ent.Create(shareRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		shareRef01Data = core.ToMapAny(entityData(shareRef01DataResult))
...
func shareBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LOCATION_SHARING_TEST_SHARE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LOCATION_SHARING_TEST_SHARE_ENTID"])
