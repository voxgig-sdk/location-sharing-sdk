# location-sharing-mcp

[MCP](https://modelcontextprotocol.io) server exposing the LocationSharing SDK as
two agent tools — `location-sharing_list` and `location-sharing_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/location-sharing-mcp)
make build

# 2. Provide credentials via the environment
export LOCATION_SHARING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user location-sharing \
  -- /absolute/path/to/location-sharing-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./location-sharing-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// location-sharing_list: first page of records
{ "entity": "building_check" }
{ "entity": "building_check", "query": { } }

// location-sharing_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/location-sharing-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user location-sharing \
     -- "$PWD"/dist/*/location-sharing-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `location-sharing_list` and `location-sharing_load` tools now appear
   in new sessions. Ask the agent to *"list building_check using location-sharing"*
   and it calls `location-sharing_list` with `{"entity":"building_check"}`.
...
```sh
export LOCATION_SHARING_APIKEY=sk_live_xxx            # API key
export LOCATION_SHARING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `location-sharing_list` tool
...
### Call the `location-sharing_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `location-sharing_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `location-sharing_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LOCATION_SHARING_APIKEY` | API key sent with every request. |
| `LOCATION_SHARING_BASE` | Optional override of the API base URL. |

### Entities

The 9 entities valid as the `entity` argument:

address | building_check | export | history | location | marker | repeat | search | share

### Smoke test via HTTP (raw JSON-RPC)

```sh
./location-sharing-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"location-sharing_load","arguments":{"entity":"address","query":{"id":1}}}}'
```
