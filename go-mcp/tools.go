...
type Args struct {
	Entity string         `json:"entity" jsonschema:"address | building_check | export | history | location | marker | repeat | search | share"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.LocationSharingSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "location-sharing_list",
		Description: "List records from LocationSharing. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "location-sharing_load",
		Description: "Load a single record from LocationSharing. " +
...
func runOp(client *sdk.LocationSharingSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.LocationSharingSDK, name string) (sdk.LocationSharingEntity, error) {
	switch strings.ToLower(name) {
	case "address":
		return client.Address(nil), nil
	case "building_check":
		return client.BuildingCheck(nil), nil
	case "export":
	...
	case "marker":
		return client.Marker(nil), nil
	case "repeat":
		return client.Repeat(nil), nil
	case "search":
		return client.Search(nil), nil
	case "share":
		return client.Share(nil), nil
