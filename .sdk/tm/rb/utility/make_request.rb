...
module LocationSharingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LocationSharingResponse.new({})
    ...
    if fetch_err
      ...
      response = LocationSharingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LocationSharingResponse.new(fetched)
    else
