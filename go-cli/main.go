// Command LocationSharing-cli is a CLI + REPL for the LocationSharing SDK,
...
const prompt = "location-sharing"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "address building_check export history location marker repeat search share"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LOCATION_SHARING_APIKEY (api key), LOCATION_SHARING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LOCATION_SHARING_APIKEY carries the API key and
	// LOCATION_SHARING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LOCATION_SHARING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LOCATION_SHARING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLocationSharingSDK(opts)
