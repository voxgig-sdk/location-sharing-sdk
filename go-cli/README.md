# location-sharing-cli

boru-driven command-line client **and** interactive REPL for the LocationSharing
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/location-sharing-cli)
make build

# 2. See usage (words, entities, env vars)
./location-sharing-cli --help

# 3. Provide credentials once, via the environment
export LOCATION_SHARING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./location-sharing-cli load 1 address            # {id:1} shorthand
./location-sharing-cli load '{id:1}' address       # explicit match map

# 5. Override the API base URL for a single call
LOCATION_SHARING_BASE=https://api.example.com ./location-sharing-cli load 1 address

# 6. No arguments -> interactive REPL
./location-sharing-cli
location-sharing> load 1 address
location-sharing> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/location-sharing-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/location-sharing-cli load 1 address
   ```
...
```sh
./location-sharing-cli load 1 address          # scalar shorthand for {id:1}
./location-sharing-cli load '{id:1}' address     # explicit match map
```
...
./location-sharing-cli load 1 address
```
...
Run with no arguments to open a REPL (prompt `location-sharing>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/location-sharing-cli`. |
...
(`./location-sharing-cli <expr>`) and interactively (the REPL), and expressions compose the
