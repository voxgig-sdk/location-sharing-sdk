...
The Python SDK for the LocationSharing API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Address(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.LocationSharing(None).load(
    {"id": "test01"}, None
)
