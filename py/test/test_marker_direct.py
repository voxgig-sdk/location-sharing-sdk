...
class TestMarkerDirect:

    def test_should_direct_list_marker(self):
        setup = _marker_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-marker", "live" if setup["live"] else "unit")
        if _skip:
...
def _marker_direct_setup(mockres):
