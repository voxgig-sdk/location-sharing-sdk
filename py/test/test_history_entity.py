...
class TestHistoryEntity:
    ...
    def test_should_stream(self):
        ...
        base = LocationSharingSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOCATION_SHARING_TEST_HISTORY_ENTID JSON to run live")
        client = setup["client"]
        ...
        history_ref01_data = helpers.to_map(runner.entity_data(history_ref01_ent.create(history_ref01_data, None)))
        assert history_ref01_data is not None
        assert history_ref01_data["id"] is not None
        ...
        history_ref01_ent.remove(history_ref01_match_rm0, None)

        # LIST
        history_ref01_match_rt0 = {}

        history_ref01_list_rt0_result = history_ref01_ent.list(history_ref01_match_rt0, None)
        assert isinstance(history_ref01_list_rt0_result, list)
...
def _history_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOCATION_SHARING_TEST_HISTORY_ENTID"))
