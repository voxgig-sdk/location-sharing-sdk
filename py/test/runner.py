# LocationSharing SDK test runner
...
class LocationSharingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LocationSharingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LocationSharingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LocationSharingTestRunner.getenv("LOCATION_SHARING_TEST_LIVE")
        override = LocationSharingTestRunner.getenv("LOCATION_SHARING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LocationSharingTestRunner.getenv(key)
        ...
        explain = LocationSharingTestRunner.getenv("LOCATION_SHARING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if LocationSharingTestRunner._test_control is not None:
            return LocationSharingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LocationSharingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LocationSharingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LocationSharingTestRunner.load_test_control()
...
def load_env_local():
    LocationSharingTestRunner.load_env_local()


def env_override(m):
    return LocationSharingTestRunner.env_override(m)


def entity_data(v):
    return LocationSharingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LocationSharingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LocationSharingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LocationSharingTestRunner.load_test_control()


def live_client_options():
    return LocationSharingTestRunner.live_client_options()


def live_delay_ms():
    return LocationSharingTestRunner.live_delay_ms()
