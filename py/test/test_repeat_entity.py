    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOCATION_SHARING_TEST_REPEAT_ENTID JSON to run live")
        client = setup["client"]
        ...
        repeat_ref01_data = helpers.to_map(runner.entity_data(repeat_ref01_ent.create(repeat_ref01_data, None)))
...
def _repeat_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "LOCATION_SHARING_TEST_REPEAT_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOCATION_SHARING_TEST_REPEAT_ENTID"))
