...
class TestBuildingCheckEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.BuildingCheck(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.BuildingCheck(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOCATION_SHARING_TEST_BUILDING_CHECK_ENTID JSON to run live")
        client = setup["client"]
        ...
        building_check_ref01_list_result = building_check_ref01_ent.list(building_check_ref01_match, None)
...
def _building_check_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOCATION_SHARING_TEST_BUILDING_CHECK_ENTID"))
