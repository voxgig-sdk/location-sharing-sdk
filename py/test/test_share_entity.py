    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOCATIONSHARING_TEST_SHARE_ENTID JSON to run live")
        client = setup["client"]
        ...
        share_ref01_data = helpers.to_map(share_ref01_ent.create(share_ref01_data, None))
...
def _share_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "LOCATIONSHARING_TEST_SHARE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOCATIONSHARING_TEST_SHARE_ENTID"))
