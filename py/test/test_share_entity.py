...
class TestShareEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Share(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _share_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "share." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOCATION_SHARING_TEST_SHARE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        share_ref01_ent = client.Share(None)
        share_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.share"), "share_ref01"))

        share_ref01_data = helpers.to_map(runner.entity_data(share_ref01_ent.create(share_ref01_data, None)))
        assert share_ref01_data is not None



def _share_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/share/ShareTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "LOCATION_SHARING_TEST_SHARE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOCATION_SHARING_TEST_SHARE_ENTID"))
