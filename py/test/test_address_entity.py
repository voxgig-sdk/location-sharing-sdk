    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOCATIONSHARING_TEST_ADDRESS_ENTID JSON to run live")
        client = setup["client"]
        ...
        address_ref01_data_dt0_loaded, err = address_ref01_ent.load(address_ref01_match_dt0, None)
        assert err is None
...
def _address_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOCATIONSHARING_TEST_ADDRESS_ENTID"))
