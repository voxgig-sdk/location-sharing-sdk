...
class TestMarkerEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Marker(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Marker(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOCATION_SHARING_TEST_MARKER_ENTID JSON to run live")
        client = setup["client"]
        ...
        marker_ref01_data = helpers.to_map(runner.entity_data(marker_ref01_ent.create(marker_ref01_data, None)))
...
def _marker_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOCATION_SHARING_TEST_MARKER_ENTID"))
