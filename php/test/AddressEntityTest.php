...
class AddressEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LOCATIONSHARING_TEST_ADDRESS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$address_ref01_data_dt0_loaded, $err] = $address_ref01_ent->load($address_ref01_match_dt0, null);
        $this->assertNull($err);
...
function address_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["address01", "address02", "address03"] as $k) {
    ...
    $entid_env_raw = getenv("LOCATIONSHARING_TEST_ADDRESS_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["LOCATIONSHARING_TEST_ADDRESS_ENTID"]);
