...
class BuildingCheckEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LocationSharingSDK::test(null, null);
        $ent = $testsdk->BuildingCheck(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->BuildingCheck(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->BuildingCheck(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = building_check_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "building_check." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $building_check_ref01_data = null;
        if (count($building_check_ref01_data_raw) > 0) {
...
function building_check_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/building_check/BuildingCheckTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["building_check01", "building_check02", "building_check03"] as $k) {
