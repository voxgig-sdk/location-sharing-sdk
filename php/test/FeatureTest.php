...
// LocationSharing SDK feature test
//
...
class FtCtrl extends LocationSharingControl
{
...
class FtClient
{
    ...
    private LocationSharingUtility $utility;

    public function __construct(LocationSharingUtility $utility, array $options)
    {
    ...
    public function get_utility(): LocationSharingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LocationSharingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LocationSharingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LocationSharingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LocationSharingConfig::make_feature($name);
    ...
    private static function buildUrl(LocationSharingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LocationSharingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LocationSharingResponse($fetched);
        }
    ...
    private function fail(LocationSharingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LocationSharingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
