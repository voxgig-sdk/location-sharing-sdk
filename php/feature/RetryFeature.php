...
// LocationSharing SDK retry feature
...
class LocationSharingRetryFeature extends LocationSharingBaseFeature
{
    ...
    public function init(LocationSharingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LocationSharingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LocationSharingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LocationSharingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
