...
class ExportDirectTest < Minitest::Test
  def test_direct_load_export
    setup = export_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-export", setup[:live] ? "live" : "unit")
    if _should_skip
...
def export_direct_setup(mockres)
