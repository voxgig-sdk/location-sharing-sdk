...
class BuildingCheckDirectTest < Minitest::Test
  def test_direct_list_building_check
    setup = building_check_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-building_check", setup[:live] ? "live" : "unit")
    if _should_skip
...
def building_check_direct_setup(mockres)
