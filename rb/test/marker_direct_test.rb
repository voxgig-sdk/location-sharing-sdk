...
class MarkerDirectTest < Minitest::Test
  def test_direct_list_marker
    setup = marker_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-marker", setup[:live] ? "live" : "unit")
    if _should_skip
...
def marker_direct_setup(mockres)
