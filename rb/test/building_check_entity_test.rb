...
class BuildingCheckEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.BuildingCheck(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = LocationSharingConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = LocationSharingSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.BuildingCheck(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = building_check_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "building_check." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LOCATION_SHARING_TEST_BUILDING_CHECK_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    building_check_ref01_data = nil
    if building_check_ref01_data_raw.length > 0
...
def building_check_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "building_check", "BuildingCheckTestData.json")
  ...
  entid_env_raw = ENV["LOCATION_SHARING_TEST_BUILDING_CHECK_ENTID"]
