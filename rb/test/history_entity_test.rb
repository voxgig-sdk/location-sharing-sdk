...
class HistoryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LOCATION_SHARING_TEST_HISTORY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    history_ref01_data = Helpers.to_map(history_ref01_data_result.respond_to?(:data_get) ? history_ref01_data_result.data_get : history_ref01_data_result)
...
def history_basic_setup(extra)
  ...
  entid_env_raw = ENV["LOCATION_SHARING_TEST_HISTORY_ENTID"]
