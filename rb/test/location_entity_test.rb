...
class LocationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LOCATION_SHARING_TEST_LOCATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    assert !location_ref01_data_dt0_loaded.nil?
...
def location_basic_setup(extra)
  ...
  entid_env_raw = ENV["LOCATION_SHARING_TEST_LOCATION_ENTID"]
