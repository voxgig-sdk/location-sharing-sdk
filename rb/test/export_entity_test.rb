...
class ExportEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = export_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "export." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LOCATION_SHARING_TEST_EXPORT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    export_ref01_data = nil
    if export_ref01_data_raw.length > 0
...
def export_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "export", "ExportTestData.json")
  ...
  entid_env_raw = ENV["LOCATION_SHARING_TEST_EXPORT_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["LOCATION_SHARING_TEST_EXPORT_ENTID"])
