...
class ExportEntity
  ...
  # @return [Export, Hash] the current Export data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Export fields)
  def match_get
  ...
  #
  # @param reqmatch [ExportLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Export.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Export, Hash] the loaded Export; raises LocationSharingError on failure
