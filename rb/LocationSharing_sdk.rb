...
require_relative 'LocationSharing_types'
...
class LocationSharingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = LocationSharingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = LocationSharingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, LocationSharingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    LocationSharingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = LocationSharingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = LocationSharingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = LocationSharingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue LocationSharingError => err
    ...
    if fetched.is_a?(Hash)
      status = LocationSharingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = LocationSharingError.new(
        "graphql_error", "LocationSharingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = LocationSharingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
