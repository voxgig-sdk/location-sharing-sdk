...
//     (\`new LocationSharingSDK(...)\` / \`LocationSharingSDK.test(...)\` become
//     \`LocationSharingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'LocationSharingSDK'
...
const TEST_SEED = {"entity":{"address":{"test01":{"id":"test01"}},"building_check":{"test01":{"id":"test01"}},"export":{"test01":{"id":"test01"}},"history":{"test01":{"id":"test01"}},"location":{"test01":{"id":"test01"}},"marker":{"test01":{"id":"test01"}},"repeat":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"share":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new LocationSharingSDK(...)\` and any
// \`LocationSharingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+LocationSharingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/LocationSharingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
