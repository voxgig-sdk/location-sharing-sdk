...
describe('BuildingCheckDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new LocationSharingSDK({
  ...
  test('direct-list-building_check', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-building_check', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LOCATION_SHARING_TEST_BUILDING_CHECK_ENTID']
